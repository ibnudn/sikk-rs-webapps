...
<div id="content">
    ...
    <div class="card shadow mb-4 mx-3">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Tambah Kapasitas <?= $faskes['nama_faskes'] ?></h6>
        </div>
        <div class="card-body">
            <form action="<?= base_url('pegawai/c_kapasitas/tambah_kapasitas/'. $faskes['id_faskes']); ?>" method="post">
    ...
    <a class="btn btn-secondary mx-3" href="<?= base_url('pegawai/c_kapasitas') ?>">
...
var base_url = "<?= base_url('pegawai/c_kapasitas/'); ?>";
let faskes = <?= json_encode($faskes); ?>;
let _id_faskes = <?= $id_faskes = $this->session->id_faskes; ?>;
...
function addRow(response, error_message, class_value, capacity_value, i){
	...
	// inp = `<input type="text" class="form-control" name="kapasitas[`+i+`]" id="kapasitas[`+i+`]">`;
	// Object.keys(_resp).forEach(data => {
	// 	opt[i] = `<option value="`+ _resp[data].id_kelas +`">`+ _resp[data].nama_kelas +`</option>`;
	// 	// console.log(opt[i]);
	// 		$('#__kelas['+i+']').append(opt[i]);
	// 		i++;
	// });
	// $('#_input').append(inp);
	// $('#table_body').append(`
