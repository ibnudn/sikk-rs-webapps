...
class C_Ketersediaan extends CI_Controller {
    ...
    public function index() {
        ...
        // $data['ketersediaan'] = $this->ketersediaan->get_ketersediaan();
        $data['ketersediaan'] = $this->ketersediaan->get_joinKapasitas();
        ...
        $this->load->view('admin/ketersediaan/v_ketersediaan', $data);
        $this->load->view('templates/content_footer');
        $this->load->view('templates/footer');
    }

    public function detail_ketersediaan($id_faskes) {
        ...
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar');
        $this->load->view('admin/ketersediaan/v_detailketersediaan', $data);
        $this->load->view('templates/content_footer');
        $this->load->view('templates/footer');
    }
    ...
    public function update_ketersediaan($id_faskes) {
        ...
        if ($this->form_validation->run() == false) {
            $data = array('error_message' => form_error('ketersediaan[]'));
			$data['token_name'] = $this->security->get_csrf_token_name();
			$data['token_hash'] = $this->security->get_csrf_hash();
            echo json_encode($data);
        } else {
            ...
            '<div class="alert alert-success alert-dismissible fade show mx-3" role="alert">
                ...
                // redirect('admin/c_ketersediaan/detail_ketersediaan/'.$id_faskes);
        }
    }
    ...
    public function tambah_ketersediaan($id_faskes) {
		...
		*/
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/topbar');
            $this->load->view('admin/ketersediaan/v_tambahketersediaan', $data);
            $this->load->view('templates/content_footer');
            $this->load->view('templates/footer');
        } else {
    ...
    public function tambah_ketersediaan_faskes() {
		$data['title'] = "Tambah Ketersediaan Faskes - SIKK RS";
        $data['menu'] = $this->menu->menu();
        $data['faskes'] = $this->faskes->join_kapasitas();
        $data['ketersediaan'] = $this->ketersediaan->get_ketersediaan();
        // print_r($this->db->last_query());
		
        /* 
        * ----------------------------------------------
        * Membandingkan faskes yang ketersediaannya sudah ada
        * -----------------------------------------------
        */ 
        $faskes_ada = array();
        foreach($data['ketersediaan'] as $row) {
			$faskes_ada[] = array(
        ...
        $data['faskes_kosong'] = array_udiff($faskes, $faskes_ada, 'perbandingan_faskes');
        /* ------------------------------------------------------------------------ */
        ...
        // $this->form_validation->set_rules('ketersediaan[]', 'Ketersediaan', 'required|trim|less_than_equal_to['.$this->input->post('kapasitas').']', [
			...
			$this->form_validation->set_rules(
				...
				'required|trim|less_than_equal_to[kapasitas]', 
				array(
					'required' => '%s perlu diisi',
					'less_than_equal_to' => '%s melebihi kapasitas'
					)
				...
				if ($this->form_validation->run() == false) {
					...
					$this->load->view('templates/sidebar', $data);
					$this->load->view('templates/topbar');
					$this->load->view('admin/ketersediaan/v_tambahketersediaanfaskes', $data);
					$this->load->view('templates/content_footer');
					$this->load->view('templates/footer');
				} else {
					$this->ketersediaan->tambah_ketersediaan();
					...
					redirect('admin/c_ketersediaan');
				}
			}
			
			public function hapus_ketersediaan($id_faskes, $id_kelas) {
        $this->ketersediaan->hapus_ketersediaan($id_faskes, $id_kelas);
        $this->session->set_flashdata('pesan', 
        '<div class="alert alert-success alert-dismissible fade show mx-3" role="alert">
            Data Ketersediaan berhasil dihapus!
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>');
        redirect('admin/c_ketersediaan/detail_ketersediaan/'. $id_faskes);
    }
	...
	public function kapasitas_check($kapasitas, $ketersediaan)
	{
		$kapasitas = $this->input->post("kapasitas");
		if ($ketersediaan <= $kapasitas) {
			return TRUE;
		} else {
			$this->form_validation->set_message('kapasitas_check', '%s melebihi kapasitas');
			return FALSE;
		}
	}
